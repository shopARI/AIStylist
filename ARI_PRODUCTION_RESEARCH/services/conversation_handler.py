...
When users ask "what time is it" or "what's the date", you MUST use the information above. Do not say you don't have access to real-time information - you have it right here."""
    ...
    async def _get_interactions_from_redis(self, session_id: str) -> List[ProductInteraction]:
        """Get product interactions from Redis - DISABLED (memory-only mode)."""
        # DISABLED: Product interactions are not stored in Redis, only in memory
        # Return empty list since interactions are maintained in self.interactions[session_id]
        logger.debug(f"Product interaction Redis retrieval disabled - using memory-only storage for session {session_id}")
        return []
    
    async def _save_interactions_to_redis(self, session_id: str, interactions: List[ProductInteraction]) -> bool:
        """Save product interactions to Redis - DISABLED (memory-only mode)."""
        # DISABLED: Product interactions are kept in memory only, not stored in Redis
        logger.debug(f"Product interaction Redis storage disabled - {len(interactions)} interactions kept in memory for session {session_id}")
