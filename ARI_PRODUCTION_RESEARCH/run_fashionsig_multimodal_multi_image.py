#!/usr/bin/env python3
"""
A100 Optimized FashionSigLIP MULTIMODAL Multi-Image Processing
...
- Preserves individual image visual + text information
- Collection: fashion_fashionsig_multimodal_multi

MULTIMODAL EMBEDDINGS (2048 dimensions):
- Uses full SiglipModel (vision + text encoders)
- Generates 1024-dim text embeddings from product descriptions
- Generates 1024-dim vision embeddings from product images
- Concatenates to 2048-dim multimodal embeddings per image
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(f'fashionsig_a100_multimodal_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')
    ]
)
logger = logging.getLogger("fashionsig_a100_multimodal")
...
class A100FashionSigMultimodalProcessor:
    """A100-optimized FashionSigLIP MULTIMODAL processor (text+vision 2048d)"""

    def __init__(self):
        self.model = None  # Full SiglipModel with text + vision encoders
        self.processor = None
        self.tokenizer = None  # For text encoding
        self.device = None
        self.torch = None
        self.qdrant_client = None
        # Use dedicated collection for multimodal embeddings (2048 dimensions)
        self.collection_name = "fashion_fashionsig_multimodal_multi"
        self.circuit_breaker = CircuitBreaker()

        # A100-optimized configuration (2.5K batch for multimodal with multi-image)
        self.batch_size = int(os.getenv('BATCH_SIZE', '2500'))
        ...
        self.checkpoint_file = f"fashionsig_a100_multimodal_checkpoint_{datetime.now().strftime('%Y%m%d')}.json"
        ...
        logger.info(f"A100 FashionSig MULTIMODAL Processor initialized (text+vision 2048d)")
    ...
    async def initialize_model(self):
        """Initialize FULL FashionSigLIP model (text + vision) with A100 optimization"""
        try:
            logger.info("Loading FULL FashionSigLIP MULTIMODAL model for A100...")
            ...
            # Load FULL multimodal model with A100 optimizations
            model_name = "google/siglip-large-patch16-384"

            # Load full model (vision + text encoders)
            ...
            if hasattr(torch, 'compile') and torch.cuda.is_available():
                logger.info("Compiling multimodal model with torch.compile for A100...")
                self.model = torch.compile(self.model, mode="max-autotune")

            logger.info("SUCCESS: FashionSigLIP MULTIMODAL model loaded (text + vision encoders)")
            logger.info("  Vision encoder: 1024 dimensions")
            logger.info("  Text encoder: 1024 dimensions")
            logger.info("  Combined output: 2048 dimensions")
            return True
    ...
    async def initialize_qdrant(self):
        ...
        try:
            ...
            if self.collection_name not in collection_names:
                ...
                # Multimodal: text (1024) + vision (1024) = 2048 dimensions
                await self.qdrant_client.create_collection(
                    collection_name=self.collection_name,
                    vectors_config=VectorParams(size=2048, distance=Distance.COSINE)
                )
                logger.info(f"SUCCESS: Multimodal collection {self.collection_name} created (2048d)")
            else:
                logger.info(f"SUCCESS: Multimodal collection {self.collection_name} already exists")
    ...
    async def generate_embeddings_batch(self, image_batch: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Generate MULTIMODAL embeddings (text + vision = 2048d) using A100-optimized batching"""
        if not image_batch or not self.model:
            return []

        try:
            # Process in sub-batches to manage A100 memory efficiently
            sub_batch_size = min(150, len(image_batch))  # 150 for multimodal (more memory intensive)
            all_results = []
            ...
            for i in range(0, len(image_batch), sub_batch_size):
                ...
                # Prepare text descriptions for text encoder
                texts = []
                for item in sub_batch:
                    product = item['product']
                    # Create rich text description for better text embeddings
                    text_desc = f"{product.get('name', '')} {product.get('category', '')} {product.get('brand', '')} {product.get('description', '')}".strip()
                    if not text_desc:
                        text_desc = f"{product.get('category', 'fashion')} item"
                ...
                # Process texts through text encoder (SigLIP max is 64 tokens)
                text_inputs = self.tokenizer(texts, return_tensors="pt", padding=True, truncation=True, max_length=64)
                text_inputs = {k: v.to(self.device) for k, v in text_inputs.items()}

                with self.torch.no_grad():
                    with self.torch.amp.autocast('cuda'):  # Mixed precision for A100
                        # Get vision embeddings (1024d)
                        ...
                        # Get text embeddings (1024d)
                        text_outputs = self.model.text_model(**text_inputs)
                        text_embeddings = text_outputs.last_hidden_state.mean(dim=1)  # [batch, 1024]

                        # Concatenate text + vision = 2048d multimodal embedding
                        multimodal_embeddings = self.torch.cat([text_embeddings, vision_embeddings], dim=1)  # [batch, 2048]

                # Convert to CPU numpy
                multimodal_embeddings_np = multimodal_embeddings.cpu().numpy()

                # Prepare results - each image gets its own multimodal embedding
                for j, item in enumerate(sub_batch):
                    all_results.append({
                        'product_uuid': item['product_uuid'],
                        'image_index': item['image_index'],  # Preserve image index
                        'embedding': multimodal_embeddings_np[j],  # 2048d vector
                        'product': item['product'],
                        'text_used': texts[j]  # Store text for debugging
                    })

                # Memory cleanup
                del image_inputs, text_inputs, vision_outputs, text_outputs
                del vision_embeddings, text_embeddings, multimodal_embeddings, multimodal_embeddings_np
                self.torch.cuda.empty_cache()

            logger.debug(f"Generated {len(all_results)} multimodal embeddings (2048d, separate per image)")
            return all_results

        except Exception as e:
            logger.error(f"ERROR: Multimodal embedding generation failed: {e}")
            traceback.print_exc()
            return []

    async def upload_to_qdrant_batch(self, embedding_batch: List[Dict[str, Any]]) -> bool:
        """Upload multimodal embeddings to Qdrant with UUID-based point IDs (chunked for reliability)"""
        ...
        try:
            ...
            for item in embedding_batch:
                ...
                # Create unique point ID as UUID string with image index
            ...
            # Upload in smaller chunks to avoid timeout (max 500 points per chunk)
            chunk_size = 500
            total_uploaded = 0
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        if operation_info.status == "completed":
                            ...
                            logger.debug(f"Uploaded chunk {i//chunk_size + 1}: {len(chunk)} points ({total_uploaded}/{len(points)} total)")
                            break
                        else:
                            logger.warning(f"Chunk {i//chunk_size + 1} upload status: {operation_info.status}")
                        ...
                        logger.warning(f"Chunk {i//chunk_size + 1} upload attempt {attempt + 1} failed: {chunk_error}")
                        if attempt < max_retries - 1:
                            await asyncio.sleep(2 ** attempt)  # Exponential backoff
                            continue
                        else:
            ...
            logger.info(f"Successfully uploaded {total_uploaded} multimodal embeddings to Qdrant")
            return True
    ...
    async def run_production_processing(self) -> bool:
        ...
        try:
            ...
            # Allow starting from a specific offset (useful for resuming from specific point)
            start_offset = int(os.getenv('START_OFFSET', '0'))
            processed_offset = self.stats.last_processed_id or start_offset

            if start_offset > 0:
                logger.info(f"Starting from custom offset: {start_offset:,} (START_OFFSET env var)")
                self.stats.total_processed = start_offset
                self.stats.last_processed_id = start_offset
...
async def main():
    ...
    processor = A100FashionSigMultimodalProcessor()
