"""
Neo4j Tools for CrewAI Migration
Wraps existing Neo4j functionality into CrewAI-compatible tools.
"""
import os
import logging
import asyncio
...
def neo4j_query_tool(cypher: str, parameters: Dict[str, Any] = None) -> List[Dict]:
    """
    ...
    try:
        ...
        logger.info(f"Neo4j query returned {len(results)} records")
        return results
...
def semantic_expansion_tool(query: str, context: Dict[str, Any] = None) -> Dict[str, Any]:
    """
    ...
    Example:
        expansion = semantic_expansion_tool(
...
@tool("Neo4j Fulltext Search")
def neo4j_fulltext_search_tool(
    search_string: str,
    limit: int = 10,
    filters: Dict[str, Any] = None
) -> List[Dict]:
    """
    Execute fulltext search against Neo4j product index.
    ...
    try:
        ...
        # Execute query using neo4j_query_tool
        results = neo4j_query_tool(cypher, parameters)
        ...
        if products:
            logger.info(f"Fulltext search returned {len(products)} products")
            return products
    ...
    try:
        ...
        results = neo4j_query_tool(cypher_fallback, parameters)
        ...
        logger.info(f"CONTAINS search returned {len(products)} products")
        return products

    except Exception as e:
        logger.error(f"Both fulltext and CONTAINS search failed: {e}")
        return []
