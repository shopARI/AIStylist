...
Modernized with CAMEL 0.2.7, RolePlay, and Memory
"""
...
# Import modernized agents
from agents.cypher_bot_modernized import CypherBotModernized
from agents.vibe_bot_modernized import VibeBotModernized
from agents.judge_modernized import JudgeAriModernized

# CAMEL 0.2.7 imports for backward compatibility
...
class CypherBotAgent:
    """
    Wrapper for modernized CypherBot with CAMEL 0.2.7, RolePlay, and memory.
    Maintains compatibility with battle system while using enhanced features.
    """
    
    def __init__(self, neo4j_client, model_type: str = "gpt-4o-mini"):
        """Initialize CypherBot with modernized agent"""
        self.neo4j_client = neo4j_client
        self.model_type = model_type
        
        # Use modernized CypherBot
        self.agent = CypherBotModernized(
            neo4j_client=neo4j_client,
            query_timeout=120.0
        )
        
        logger.info(f"CypherBot initialized with modernized CAMEL 0.2.7 agent")
...
Be confident in your graph-based approach."""
    ...
    async def search(
        ...
        """
        Search for products using modernized CypherBot with CAMEL 0.2.7 features
        """
        try:
            # Use modernized agent's search method
            results = await self.agent.search(
                query=query,
                limit=limit,
                filters=filters,
            ...
            # Add battle-specific metadata
            for product in results:
                product['battle_agent'] = 'cypher'
                product['modernized'] = True
                product['camel_enhanced'] = True
            
            logger.info(f"CypherBot (Modernized) found {len(results)} products")
            return results
            
        except Exception as e:
            logger.error(f"CypherBot modernized search error: {e}")
            return []
...
class VibeBotAgent:
    """
    Wrapper for modernized VibeBot with CAMEL 0.2.7, RolePlay, and aesthetic intelligence.
    Maintains compatibility with battle system while using enhanced features.
    """
    
    def __init__(self, qdrant_client, model_type: str = "gpt-4o-mini"):
        """Initialize VibeBot with modernized agent"""
        self.qdrant_client = qdrant_client
        self.model_type = model_type
        
        # Use modernized VibeBot
        self.agent = VibeBotModernized(qdrant_client=qdrant_client)
        
        logger.info(f"VibeBot initialized with modernized CAMEL 0.2.7 agent")
...
Be confident in your semantic and style-based approach."""
    ...
    async def search(
        ...
        """
        Search for products using modernized VibeBot with CAMEL 0.2.7 features
        """
        try:
            # Use modernized agent's search method
            results = await self.agent.search(
            ...
            # Add battle-specific metadata
            for product in results:
                product['battle_agent'] = 'vibe'
                product['modernized'] = True
                product['camel_enhanced'] = True
            
            logger.info(f"VibeBot (Modernized) found {len(results)} products")
            return results
            
        except Exception as e:
            logger.error(f"VibeBot modernized search error: {e}")
            return []
...
class JudgeAriAgent:
    """
    Wrapper for modernized Judge Ari with CAMEL 0.2.7, RolePlay, and memory.
    Maintains compatibility with battle system while using enhanced features.
    """

    def __init__(self, model_type: str = "gpt-4o-mini"):
        """Initialize Judge Ari with modernized agent"""
        self.model_type = model_type

        # Use modernized Judge Ari
        self.agent = JudgeAriModernized()

        logger.info(f"Judge Ari initialized with modernized CAMEL 0.2.7 agent")
...
Important: Return your evaluation as a structured JSON object. For example:
{
    ...
    async def evaluate(
        ...
        """
        Evaluate results from both agents using modernized Judge Ari with CAMEL 0.2.7 features
        """
        try:
            # Use modernized agent's evaluate method
            judgment = await self.agent.evaluate(
            ...
            # Add battle-specific metadata
            judgment['modernized'] = True
            judgment['camel_enhanced'] = True
            judgment['features_used'] = ['RolePlay', 'Memory', 'Learning']

            logger.info(f"Judge Ari (Modernized) selected {len(judgment.get('selected_products', []))} products")
            logger.info(f"Winner: {judgment.get('winner', 'unknown')}")

            return judgment

        except Exception as e:
            logger.error(f"Judge Ari modernized evaluation error: {e}")

            # Fallback: combine both results with modernized flag
            all_products = cypher_results[:limit//2] + vibe_results[:limit//2]
            return {
                "winner": "tie",
                "reason": "Error in modernized evaluation, using balanced selection",
