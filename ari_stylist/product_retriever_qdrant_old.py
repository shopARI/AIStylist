...
from camel.retrievers import AutoRetriever, RetrieverConfig
...
class ProductRetriever:
    ...
    def __init__(self, vector_storage_path="product_data/embeddings", 
        ...
        try:
            ...
            # Initialize retriever for local mode only - UPDATED for new API
            if not self.using_remote:
                # Create a RetrieverConfig with the appropriate parameters
                retriever_config = RetrieverConfig(
                    vector_store_config={
                        "local_path": vector_storage_path,
                ...
                logger.info("AutoRetriever initialized with new API")
            else:
    ...
    def setup_product_indexing(self, neo4j_connection):
        ...
        Tracks already indexed products to avoid duplicates.
        ...
        if (self.using_remote and not hasattr(self, 'qdrant_client')) or \
           (not self.using_remote and not self.retriever) or \
        ...
        try:
            ...
            if self.using_remote:
                ...
                print(f"Found {len(indexed_product_ids)} products already indexed in Qdrant")
                ...
                print(f"Skipping {skipped_count} already indexed products")
                print(f"Need to index {len(products_to_index)} new products")
                ...
                # Process the file with the retriever - UPDATED CODE
                try:
                    logger.info(f"Indexing content from {descriptions_file} with AutoRetriever")
                    # Using the updated API method
                    self.retriever.index_from_file(descriptions_file)
                    logger.info("Local indexing complete!")
                except Exception as e:
                    logger.error(f"Error indexing with AutoRetriever: {e}")
    ...
    def _get_indexed_product_ids(self):
        ...
        try:
            ...
            print(f"Collection has {total_points} points, retrieving product IDs...")
            ...
            while True:
                ...
                # Print progress
                print(f"Retrieved {len(product_ids)} product IDs so far...")
                
                # Exit if we're out of points
                if offset is None or len(batch_points) < limit:
                    break
            
            print(f"Retrieved {len(product_ids)} unique product IDs from Qdrant")
            return product_ids
    ...
    def _index_to_remote_qdrant(self, products):
        ...
        try:
            ...
            print(f"Starting indexing of {total_products} products...")
            ...
            for i in range(0, total_products, batch_size):
                ...
                print(f"Progress: {indexed_count}/{total_products} products ({progress_pct:.1f}%)")
                ...
                print(f"Preparing batch {i//batch_size + 1} ({batch_count} products)...")
                points = []
                
                for idx, product in enumerate(batch):
                    # Print occasional item details
                    if idx % 10 == 0:
                        print(f"  - Processing product {product.get('id', '')}: {product.get('title', '')}")
                    
                    # Create text representation
                    text_repr = self._create_product_text_representation(product)
                    
                    # Generate embedding using embed_list method
                    print(f"  - Generating embedding for product {i+idx+1}/{total_products}...")
                    embedding = self.embedding_model.embed_list([text_repr])[0]
                    
                    # Create point with a UUID as the ID
                ...
                print(f"Batch {i//batch_size + 1} complete! Progress: {indexed_count}/{total_products} products ({(indexed_count/total_products)*100:.1f}%)")
            
            print(f"✅ Indexing complete! {total_products} products indexed to Qdrant collection '{self.qdrant_collection_name}'")
            logger.info(f"Remote indexing complete! {total_products} products indexed")
    ...
    def _search_remote_qdrant(self, query, limit=5, similarity_threshold=0.7):
        ...
        try:
            ...
            # Generate embedding for query using embed_list method
    ...
    def _create_product_text_representation(self, product):
        ...
        Ensures the text stays under the token limit for embedding (8192 tokens).
        ...
        # Additional safety truncation - limit to ~4000 chars (well under 8192 tokens)
        if len(text) > 4000:
            print(f"Warning: Truncating very long product text for ID {product.get('id', '')}")
            text = text[:3997] + "..."
    ...
    def _alternative_indexing(self, descriptions_file):
        ...
        try:
            ...
            # Just log that the file is ready for manual processing
            logger.info("Vector indexing skipped due to API compatibility issues")
            logger.info(f"Products description file created at: {descriptions_file}")
            logger.info("System will function with limited search capabilities")
        ...
        if isinstance(retrieved_info, list):
            ...
            for item in retrieved_info:
                if isinstance(item, dict) and 'content' in item:
                    ...
                    if product_info:
                        formatted_results.append(product_info)
                elif hasattr(item, 'content'):  # Handle new retriever result object format
                    product_info = self._parse_product_from_text(item.content)
                    if product_info:
                        formatted_results.append(product_info)
